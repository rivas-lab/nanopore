...
9. HAP - haplotype string in the interval [bimStart, bimEnd). 0 indicates reference allele and 1 indicates minor allele.
...
def find_haplotype(read, read_poly_mm, bim):
    ...
    read_hap = np.array([1 if i in set(read_poly_mm_bim_id) else 0
...
def bamToHap(fasta, bim, bam, quality_threshold = 14):
    ...
    for x in bam:
        r = Read(x, fasta)
        try:
            read_bim_l, read_bim_r, read_error_rate, read_hap_str = process_read(r, bim, quality_threshold)
        except KeyError as e:
            logger_bamToHap.WARNING('{} KeyError {}'.format(r.query_name, e))
        except Exception as e:
            logger_bamToHap.WARNING('{} {} {}'.format(r.query_name, type(e), e))
        else:
...
def bamToHap_main():
    ...
    bamToHap(fasta, bim, bam, quality_threshold = args.q)
